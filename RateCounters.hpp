...
public:
		...
		float intervalSeconds = 1.0f)
	{
		...
		auto requestedMs = std::chrono::milliseconds(static_cast<int64_t>(
			std::round(1000.0f * intervalSeconds)));

		// Snap to closest allowedMs value
		std::array<std::chrono::milliseconds, 4> allowedMs = {
			1000ms, 250ms, 100ms, 50ms,
		};
		...
		auto interval = std::chrono::milliseconds(static_cast<int64_t>(
			std::round(1000.0f * seconds)));

		finish = std::async(std::launch::async,
			[module, downstream, interval, stopRequested = requestStop.get_future()]() mutable {
			...
			while (stopRequested.wait_for(interval) != std::future_status::ready) {
				rate_values rates;
				...
				downstream->HandleRates({
					rates.sync_rate, rates.cfd_rate, rates.tac_rate, rates.adc_rate,
				});
			}
