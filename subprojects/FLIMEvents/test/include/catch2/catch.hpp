...
#define INTERNAL_CATCH_REMOVE_PARENS_10_ARG(_0, _1, _2, _3, _4, _5, _6, _7,   \
                                            _8, _9)                           \
    INTERNAL_CATCH_REMOVE_PARENS(_0), INTERNAL_CATCH_REMOVE_PARENS_9_ARG(     \
...
class BinaryExpr : public ITransientExpression {
    ...
    void streamReconstructedExpression(std::ostream &os) const override {
        formatReconstructedExpression(os, Catch::Detail::stringify(m_lhs),
                                      m_op, Catch::Detail::stringify(m_rhs));
    }
...
template <typename T> class RepeatGenerator : public IGenerator<T> {
  ...
  public:
    ...
    bool next() override {
        ...
        // In the second case, we need to move indices forward and check that
        // we haven't run up against the end
        ++m_repeat_index;
...
struct CumulativeReporterBase : IStreamingReporter {
    ...
    bool assertionEnded(AssertionStats const &assertionStats) override {
        ...
        // Our section stack copy of the assertionResult will likely outlive
        // the temporary, so it must be expanded or discarded now to avoid
        // calling a destroyed object later.
...
struct Benchmark {
    ...
    prepare(const IConfig &cfg, Environment<FloatDuration<Clock>> env) const {
        ...
        auto &&test = Detail::run_for_at_least<Clock>(
            std::chrono::duration_cast<ClockDuration<Clock>>(run_time), 1,
            fun);
    ...
    template <typename Fun,
              typename std::enable_if<
                  !Detail::is_related<Fun, Benchmark>::value, int>::type = 0>
...
template <typename L> struct BoundFlagLambda : BoundFlagRefBase {
    ...
    auto setFlag(bool flag) -> ParserResult override {
        return LambdaInvoker<
            typename UnaryLambdaTraits<L>::ReturnType>::invoke(m_lambda, flag);
    }
};
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
    ...
    void close() override {
        TrackerBase::close();
        // Generator interface only finds out if it has another item on atual
        // move
...
void XmlEncode::encodeTo(std::ostream &os) const {
    ...
    for (std::size_t idx = 0; idx < m_str.size(); ++idx) {
        ...
        default:
            ...
            // validity, only the encoding format First check that this bytes
            // is a valid lead byte: This means that it is not encoded as 1111
            // 1XXX Or as 10XX XXXX
            if (c < 0xC0 || c >= 0xF8) {
                hexEscapeChar(os, c);
                break;
            }
      ...
      m_tablePrinter(new TablePrinter(
          config.stream(), [&config]() -> std::vector<ColumnInfo> {
              if (config.fullConfig()->benchmarkNoAnalysis()) {
                  ...
                  return {{"benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32,
                           ColumnInfo::Left},
...
void XmlReporter::benchmarkEnded(BenchmarkStats<> const &benchmarkStats) {
    m_xml.startElement("mean")
        .writeAttribute(
            "value", static_cast<uint64_t>(benchmarkStats.mean.point.count()))
        .writeAttribute(
            "lowerBound",
