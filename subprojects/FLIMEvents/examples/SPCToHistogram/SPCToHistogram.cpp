...
template <typename T> class HistogramSaver : public HistogramProcessor<T> {
  ...
  public:
    ...
    void HandleFinish(Histogram<T> &&histogram,
...
int main(int argc, char *argv[]) {
    ...
    int32_t histoBits = 8;
    Histogram<SampleType> frameHisto(histoBits, inputBits, true, width,
                                     height);
    Histogram<SampleType> cumulHisto(histoBits, inputBits, true, width,
                                     height);
    cumulHisto.Clear();
